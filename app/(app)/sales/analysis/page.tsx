...
import { Line, LineChart, Bar, BarChart, CartesianGrid, XAxis, YAxis, Pie, PieChart, Cell, Legend } from 'recharts';
...
export default function SalesAnalysisPage() {
  ...
  const fetchHourlyData = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error('Failed to fetch hourly data:', result.error);
        setHourlyData([]);
      }
    } catch (error) {
  ...
  const fetchCategoryData = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error('Failed to fetch category data:', result.error);
        setCategoryData([]);
      }
    } catch (error) {
  ...
  const avgTransactionValue = summaryTotals.transactions > 0 
    ? summaryTotals.revenue / summaryTotals.transactions 
    : 0;
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Revenue trends by {interval} interval.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoadingSales ? (

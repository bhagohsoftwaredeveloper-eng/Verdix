...
import { CalendarIcon, Printer, Eye, Download, FileText, Image as ImageIcon, X, TrendingUp, ShoppingCart, CreditCard, Users, Search } from 'lucide-react';
...
export default function OverallReadingPage() {
  ...
  const [readingData, setReadingData] = useState<OverallReadingData | null>(null);
  ...
  const fetchShifts = async () => {
    ...
    try {
      ...
      const response = await fetch(getApiUrl(`/pos/shifts?${params.toString()}`));
      const result = await response.json();

      if (result.success) {
        setShifts(result.data);
        setReadingData(null); // Clear previous reading data
        setSelectedShift(null);
        if (result.data.length === 0) {
            toast({
                title: 'Info',
                description: 'No completed shifts found for the selected criteria.',
            });
        }
      } else {
  ...
  const fetchReadingForShift = async (shift: any) => {
    setSelectedShift(shift);
    setIsLoading(true);
    try {
      const params = new URLSearchParams();
      params.append('terminalId', shift.terminal_id);
      params.append('shiftId', shift.id);

      const response = await fetch(getApiUrl(`/sales/overall-reading?${params.toString()}`));
      const result = await response.json();
      ...
      if (result.success) {
        ...
        if (result.data.transactionCount === 0) {
            toast({
                title: 'Info',
                description: `No transactions found for shift ID: ${shift.id}.`,
            });
        }
      } else {
        console.error('Failed to fetch reading for shift:', result.error);
        toast({
          title: 'Error',
          description: result.error || 'Failed to fetch reading for shift',
          variant: 'destructive',
        });
      }
    } catch (error) {
      console.error('Error fetching reading for shift:', error);
      toast({
        title: 'Error',
        description: 'Failed to fetch reading for shift',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  const handleReactToPrintFn = useReactToPrint({
    ...
    pageStyle: `
        @page {
            size: ${printerFormat === 'A4' ? 'A4 portrait' : (printerFormat === '58mm' ? '58mm' : '80mm')} auto;
            margin: 0;
        }
        @media print {
            body {
      ...
      setTimeout(async () => {
        if (previewRef.current) {
          ...
          const pdf = new jsPDF({
            ...
            unit: 'mm',
            format: printerFormat === 'A4' ? 'a4' : [pdfWidth, pdfHeight],
          });
    ...
    <div className="space-y-6 p-6 bg-slate-50/50 min-h-full rounded-xl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        ...
        <div className="flex flex-wrap items-center gap-3">
                ...
                <Popover>
                    <PopoverTrigger asChild>
                        <Button
                            variant={"ghost"}
                            className={cn(
                                "w-[240px] justify-start text-left font-normal hover:bg-transparent",
            ...
            <Button onClick={fetchShifts} disabled={isLoading} className="bg-primary hover:bg-primary/90 text-white shadow-sm gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Shifts Table - Left Column or Top on small screens */}
                ...
                <CardContent className="p-0 max-h-[600px] overflow-auto">
                    <Table>
                        <TableHeader className="bg-slate-50/50">
                            <TableRow>
                                <TableHead className="text-xs">Shift info</TableHead>
                                ...
                                shifts.map((shift) => (
                                    <TableRow 
                                        ...
                                        className={cn(
                                            ...
                                            selectedShift?.id === shift.id && "bg-blue-50/50 hover:bg-blue-50/50"
                                        )}
                                        onClick={() => fetchReadingForShift(shift)}
                                    >
                                        <TableCell>
                                            <div className="flex flex-col gap-0.5">
                                                <span className="font-medium text-slate-700 text-sm">{shift.cashier_name || 'Unknown'}</span>
                                                <span className="text-xs text-slate-500">{shift.terminal_id}</span>
        ...
        {/* Reading Details - Right Column or Bottom on small screens */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Card className="border-none shadow-sm bg-gradient-to-br from-blue-500 to-blue-600 text-white overflow-hidden relative">
                            ...
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm font-medium text-blue-100">Net Sales</CardTitle>
                            </CardHeader>
                            <CardContent>
                        ...
                        <Card className="border-none shadow-sm bg-gradient-to-br from-emerald-500 to-emerald-600 text-white overflow-hidden relative">
                            ...
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm font-medium text-emerald-100">Gross Sales</CardTitle>
                            </CardHeader>
                            <CardContent>
                        ...
                        <Card className="border-none shadow-sm bg-gradient-to-br from-violet-500 to-violet-600 text-white overflow-hidden relative">
                            ...
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm font-medium text-violet-100">Transactions</CardTitle>
                            </CardHeader>
                            <CardContent>
                        ...
                        <Card className="border-none shadow-sm bg-gradient-to-br from-amber-500 to-amber-600 text-white overflow-hidden relative">
                            ...
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm font-medium text-amber-100">Total Discounts</CardTitle>
                            </CardHeader>
                            <CardContent>

...
import { SaleItem } from './page'; // Adjust import based on where SaleItem type is defined, preferably moved to types
...
interface CancelSaleDialogProps {
  ...
  onCancelAll: () => void;
  selectedItem: any | null; // Typed loosely to avoid circular dependency issues if SaleItem is in page.tsx
}
...
export function CancelSaleDialog({
  ...
  useEffect(() => {
    if (isOpen && selectedItem) {
        setQuantity(1); // Default to 1 for quick single void, or selectedItem.quantity for full? User usually voids 1 or all. Let's default 1.
    }
  }, [isOpen, selectedItem]);
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <Input 
                              ...
                              onChange={(e) => {
                                  ...
                                  if (val > 0 && val <= (selectedItem?.quantity || 1)) {
                                      setQuantity(val);
                                  }
                              }}

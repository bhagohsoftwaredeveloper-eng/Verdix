...
export function RecordPaymentDialog({ sale }: { sale: Sale }) {
  ...
  async function onSubmit(values: PaymentFormValues) {
    ...
    toast({
      ...
      description: `Payment of ₱${values.amount.toFixed(2)} for invoice ${sale.id.substring(0,7)} has been recorded.`,
    });
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          ...
          <DialogDescription>
            For Invoice {sale.id.substring(0, 7)} - Due {sale.dueDate ? format(new Date(sale.dueDate), 'PP') : 'N/A'}
          </DialogDescription>
        </DialogHeader>
        <div className="py-2">

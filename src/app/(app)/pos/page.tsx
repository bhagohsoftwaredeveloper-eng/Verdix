...
import { AdjustPointsDialogContent } from '@/app/(app)/customer/loyalty/adjust-points-dialog';
...
export default function POSPage() {
  ...
  const handleAddItem = (product: Product | undefined) => {
    if (product) {
      ...
      if (existingItem) {
        ...
        const newItem: SaleItem = { ...product, quantity: 1, discount: 0, name: product.name };
  ...
  const handleOpenLoyalty = () => {
    if (selectedCustomer && selectedCustomer.id !== 'walk-in') {
      ...
      toast({
        ...
        description: "Please select a registered customer to view loyalty points.",
        variant: "destructive",
      });
    }
  };
  ...
  const headerActions = [
    ...
    { icon: Percent, label: 'Discount', fKey: 'F3', action: handleOpenEditDialog },
    ...
    { icon: Power, label: 'Shutdown', fKey: 'F8', action: handleLogout },
  ];
  ...
  const footerActions = [
    ...
    { icon: Ticket, label: 'Price Inquiry', action: () => setIsPriceInquiryOpen(true) },
  ];
    ...
    <>
            ...
            <div className="flex-1 bg-white border border-slate-200 rounded-lg overflow-hidden flex flex-col">
                  ...
                  <TableBody>
                    {items.map((item) => (
                      <TableRow key={item.id} className="border-slate-100" data-state={selectedItemId === item.id ? 'selected' : ''}>
                        ...
                        <TableCell className="text-right font-mono text-slate-700">₱{item.price.toFixed(2)}</TableCell>
                        ...
                        <TableCell className="text-right font-mono text-slate-700">₱{(item.price * item.quantity).toFixed(2)}</TableCell>
                        <TableCell className="text-right font-mono text-slate-700">{item.discount.toFixed(2)}%</TableCell>
                        <TableCell className="text-right font-mono text-slate-700">₱{(item.price * item.quantity * (1 - item.discount / 100)).toFixed(2)}</TableCell>
                        <TableCell>
              ...
              <div className="p-4 border-t border-slate-200 bg-slate-50/50">
                <div className="grid grid-cols-3 gap-x-8 gap-y-1 text-sm">
                  ...
                  <div className="flex justify-between text-slate-600">
                    <span>Vat Sales:</span>
                    <span className="font-mono">₱{vatSales.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-slate-600">
      ...
      {selectedCustomer && selectedCustomer.id !== 'walk-in' && (
        <Dialog open={isLoyaltyOpen} onOpenChange={setIsLoyaltyOpen}>
          <AdjustPointsDialogContent customer={{ ...selectedCustomer, loyaltyPoints: selectedCustomer.loyaltyPoints || 0 }} onFinished={() => setIsLoyaltyOpen(false)} />
        </Dialog>
      )}

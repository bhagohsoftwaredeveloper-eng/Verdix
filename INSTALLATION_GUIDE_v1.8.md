# Giya sa Pag-install sa Stock Pilot v1.8

Kini nga giya mag-tudlo kanimo kung unsaon pag-install sa Stock Pilot POS (Version 1.8) ug ang mga kinahanglanon niini.
...
2.  **MySQL Server**: 
    ...
    *   Kinahanglan naa kay database nga gingalan ug **`stock_pilot`** (or unsa mang ngalana nga naa sa imong `.env`).
...
1.  I-double click ang file nga **`StockPilotSetup_1.8.exe`**.
...
1.  Pilia ang folder diin nimo gusto i-install (by default naa sa `C:\Program Files (x86)\Stock Pilot`).
